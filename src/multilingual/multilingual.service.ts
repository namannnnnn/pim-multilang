...
export class Translator {
    ...
    onModuleInit() {
        try {
            this.googleTranslator = new TranslationServiceClient ({
                                                                    projectId:`${String(process.env.GC_PROJECT_ID)}`,
                                                                    credentials:{
                                                                                private_key:String(process.env.GC_PRIVATE_KEY),
                                                                                client_email:String(process.env.GC_CLIENT_EMAIL),
                                                                                client_id:String(process.env.GC_CLIENT_ID)      
                                                                                }
                                                                  })
        } catch(error) {
            console.log(error)
    ...
    async translation(googleTranslator, text, target_language) {
        if(typeof(text)==='string' && text!=''){
            let [translations] = await this.googleTranslator.translateText({
                parent: `projects/${process.env.GC_PROJECT_ID}/locations/global`,
                contents: [text], 
